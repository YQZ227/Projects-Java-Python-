...
public class Client {
	...
	public static void main(String [] args) throws Exception {
		...
		List<Thread> tdpool = new ArrayList<>();
		...
		double n = pwdPerReq*threadNum*reqNum;
		long hashEndTime = System.currentTimeMillis();
		System.out.println("Hashing password finished ");
		System.out.println("Throughput for hasing logRounds = " + ": " + n * 1000f/(hashEndTime-hashStartTime));
		System.out.println("Latency for hashinglogRounds = " + ": " + (hashEndTime-hashStartTime)/n);
		System.out.println("Final hashing result =" + ": " + 6/(hashEndTime-hashStartTime)/(n  * 1000f));
	}
	...
	int requestNum, int pwdPerReq) {
		...
		for (int j=1; j<=requestNum; j++ ){
			...
			// System.out.println("Throughput for checking logRounds = " + s + ": " + n * 1000f/(checkEndTime-hashEndTime));
			// System.out.println("Latency for checking logRounds = " + s + ": " + (checkEndTime-hashEndTime)/n);
			// System.out.println("Final checking result =" + s + ": " + 6/((checkEndTime-hashEndTime)/(n * 1000f)));
		}
		} catch (TException x) {
			x.printStackTrace();
		} 
